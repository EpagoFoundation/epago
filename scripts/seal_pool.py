...
def main() -> int:
    ...
    print(f'public_pool_path            = "pools/{pool_path.name}"')
    print(f'public_pool_digest          = "{digest}"')
    print(f'public_pool_manifest_path   = "pools/{manifest_path.name}"')
    print(f'public_pool_manifest_digest = "{manifest_digest}"')
    print()
    print("Paths are relative to your state directory and are NOT part of the")
    print("contract -- only the digests are. Keep the files elsewhere with:")
    print()
    print(f"  export EPAGO_EVAL_PUBLIC_POOL_PATH={pool_path.resolve()}")
    print(f"  export EPAGO_EVAL_PUBLIC_POOL_MANIFEST_PATH={manifest_path.resolve()}")
    print()
